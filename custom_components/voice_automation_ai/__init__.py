...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_create_automation(call: ServiceCall) -> dict[str, Any]:
        ...
        try:
            ...
            # Get the automation dict (handle if it's a list)
            automation_dict = automation_data[0] if isinstance(automation_data, list) else automation_data

            return {
                "success": True,
                "automation_id": automation_id,
                "alias": automation_dict.get("alias", "Unknown"),
            }
